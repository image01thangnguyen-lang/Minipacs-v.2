# Phase 3 PR1 — Contract, Index và Baseline

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE3_WORKLIST_QUERY_FILTER_GRID_PLAN.md](./DOCTOR_WORKSPACE_PHASE3_WORKLIST_QUERY_FILTER_GRID_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Chốt contract query/response, data dictionary, query budget, index strategy và baseline trước khi thay đường đọc.
...
Trọng tâm riêng: **Contract/versioning, timezone/date boundary, sort allowlist, cursor shape, EXPLAIN/query baseline và migration index additive.**
...
Targeted matrix: Contract/versioning, timezone/date boundary, sort allowlist, cursor shape, EXPLAIN/query baseline và migration index additive.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 3 PR1 — Contract, Index và Baseline.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE3_WORKLIST_QUERY_FILTER_GRID_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE3_PR1_WORKLIST_CONTRACT_INDEX_BASELINE_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Chốt contract query/response, data dictionary, query budget, index strategy và baseline trước khi thay đường đọc.
Trọng tâm: Contract/versioning, timezone/date boundary, sort allowlist, cursor shape, EXPLAIN/query baseline và migration index additive.
