# Phase 4 PR3 — Selection, Deep Link và Request Race

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE4_SEVEN_REGION_RELATED_STUDIES_PLAN.md](./DOCTOR_WORKSPACE_PHASE4_SEVEN_REGION_RELATED_STUDIES_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE4_PR2_ACCESSIBLE_SPLIT_PANE_PLAN.md](./DOCTOR_WORKSPACE_PHASE4_PR2_ACCESSIBLE_SPLIT_PANE_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Chuẩn hóa selection theo URL, back/forward, refresh, double-click viewer và chống response A ghi đè B.
...
- `dashboard/lib/workspace/selection-state.ts`
...
Trọng tâm riêng: **Deep link inaccessible, race A→B→C, pagination refresh, browser history, focus và generic not-found.**
...
Targeted matrix: Deep link inaccessible, race A→B→C, pagination refresh, browser history, focus và generic not-found.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 4 PR3 — Selection, Deep Link và Request Race.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE4_SEVEN_REGION_RELATED_STUDIES_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE4_PR3_SELECTION_DEEP_LINK_RACE_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Chuẩn hóa selection theo URL, back/forward, refresh, double-click viewer và chống response A ghi đè B.
Trọng tâm: Deep link inaccessible, race A→B→C, pagination refresh, browser history, focus và generic not-found.
