# Phase 5 PR1 — Study Workspace Detail và Allowed Actions

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE5_REPORT_CONSULTATION_INTEGRATION_PLAN.md](./DOCTOR_WORKSPACE_PHASE5_REPORT_CONSULTATION_INTEGRATION_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Tạo detail contract phân tách READ_STUDY/READ_REPORT, batch allowedActions và generic error cho vùng 7.
...
Trọng tâm riêng: **Field minimization, report permission split, scope revoke, workflow state và no-N+1.**
...
Targeted matrix: Field minimization, report permission split, scope revoke, workflow state và no-N+1.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 5 PR1 — Study Workspace Detail và Allowed Actions.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE5_REPORT_CONSULTATION_INTEGRATION_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE5_PR1_STUDY_WORKSPACE_DETAIL_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Tạo detail contract phân tách READ_STUDY/READ_REPORT, batch allowedActions và generic error cho vùng 7.
Trọng tâm: Field minimization, report permission split, scope revoke, workflow state và no-N+1.
