# Phase 7 PR4 — UAT Fixtures, Scripts và Sign-off

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE7_PERFORMANCE_UAT_CONTROLLED_ROLLOUT_PLAN.md](./DOCTOR_WORKSPACE_PHASE7_PERFORMANCE_UAT_CONTROLLED_ROLLOUT_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE7_PR3_SECURITY_ACCESSIBILITY_AUTOMATION_PLAN.md](./DOCTOR_WORKSPACE_PHASE7_PR3_SECURITY_ACCESSIBILITY_AUTOMATION_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Chuẩn hóa dữ liệu synthetic, kịch bản theo vai trò và evidence workflow có chữ ký owner.
...
Trọng tâm riêng: **No real PHI, deterministic fixtures, role/facility matrix, defect traceability và approvals.**
...
Targeted matrix: No real PHI, deterministic fixtures, role/facility matrix, defect traceability và approvals.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 7 PR4 — UAT Fixtures, Scripts và Sign-off.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE7_PERFORMANCE_UAT_CONTROLLED_ROLLOUT_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE7_PR4_UAT_FIXTURES_SIGNOFF_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Chuẩn hóa dữ liệu synthetic, kịch bản theo vai trò và evidence workflow có chữ ký owner.
Trọng tâm: No real PHI, deterministic fixtures, role/facility matrix, defect traceability và approvals.
