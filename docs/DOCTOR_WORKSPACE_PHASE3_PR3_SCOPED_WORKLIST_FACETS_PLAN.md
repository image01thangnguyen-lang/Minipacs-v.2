# Phase 3 PR3 — Scoped Worklist Facets

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE3_WORKLIST_QUERY_FILTER_GRID_PLAN.md](./DOCTOR_WORKSPACE_PHASE3_WORKLIST_QUERY_FILTER_GRID_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE3_PR2_SCOPED_WORKLIST_QUERY_PLAN.md](./DOCTOR_WORKSPACE_PHASE3_PR2_SCOPED_WORKLIST_QUERY_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Xây facet counts cho trạng thái, cây tổ chức và hội chẩn bằng cùng predicate với danh sách mà không rò metadata.
...
- `dashboard/lib/authz/scope/organization-tree.ts`
...
Trọng tâm riêng: **Self-excluding facet semantics, cross-facility leakage, count/list parity, empty hidden groups và batch query.**
...
Targeted matrix: Self-excluding facet semantics, cross-facility leakage, count/list parity, empty hidden groups và batch query.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 3 PR3 — Scoped Worklist Facets.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE3_WORKLIST_QUERY_FILTER_GRID_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE3_PR3_SCOPED_WORKLIST_FACETS_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Xây facet counts cho trạng thái, cây tổ chức và hội chẩn bằng cùng predicate với danh sách mà không rò metadata.
Trọng tâm: Self-excluding facet semantics, cross-facility leakage, count/list parity, empty hidden groups và batch query.
