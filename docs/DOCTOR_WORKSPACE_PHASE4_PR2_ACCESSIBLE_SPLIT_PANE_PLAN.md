# Phase 4 PR2 — Accessible SplitPane

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE4_SEVEN_REGION_RELATED_STUDIES_PLAN.md](./DOCTOR_WORKSPACE_PHASE4_SEVEN_REGION_RELATED_STUDIES_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE4_PR1_WORKSPACE_COMPOSITION_GRID_PLAN.md](./DOCTOR_WORKSPACE_PHASE4_PR1_WORKSPACE_COMPOSITION_GRID_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Xây splitter pointer/keyboard có min/max, responsive fallback và không tạo resize/remount loop.
...
- `dashboard/app/components/ui/SplitPane.tsx`
...
Trọng tâm riêng: **Mouse/touch/keyboard, ARIA separator, clamp, zoom, resize observer và performance.**
...
Targeted matrix: Mouse/touch/keyboard, ARIA separator, clamp, zoom, resize observer và performance.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 4 PR2 — Accessible SplitPane.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE4_SEVEN_REGION_RELATED_STUDIES_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE4_PR2_ACCESSIBLE_SPLIT_PANE_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Xây splitter pointer/keyboard có min/max, responsive fallback và không tạo resize/remount loop.
Trọng tâm: Mouse/touch/keyboard, ARIA separator, clamp, zoom, resize observer và performance.
