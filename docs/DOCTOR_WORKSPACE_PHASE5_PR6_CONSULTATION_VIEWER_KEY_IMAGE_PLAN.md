# Phase 5 PR6 — Consultation, Second Read, Viewer và Key Image

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE5_REPORT_CONSULTATION_INTEGRATION_PLAN.md](./DOCTOR_WORKSPACE_PHASE5_REPORT_CONSULTATION_INTEGRATION_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE5_PR5_REPORT_ACTION_HARDENING_PLAN.md](./DOCTOR_WORKSPACE_PHASE5_PR5_REPORT_ACTION_HARDENING_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Đưa các tác vụ ngữ cảnh vào vùng 7 qua endpoint đã scoped, không nhập nhằng second read với consultation.
...
- `dashboard/components/consultation/*`
- `viewer artifact actions`
...
Trọng tâm riêng: **Capability/lifecycle, deep link, artifact scope, unavailable services, no blob overfetch và audit.**
...
Targeted matrix: Capability/lifecycle, deep link, artifact scope, unavailable services, no blob overfetch và audit.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 5 PR6 — Consultation, Second Read, Viewer và Key Image.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE5_REPORT_CONSULTATION_INTEGRATION_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE5_PR6_CONSULTATION_VIEWER_KEY_IMAGE_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Đưa các tác vụ ngữ cảnh vào vùng 7 qua endpoint đã scoped, không nhập nhằng second read với consultation.
Trọng tâm: Capability/lifecycle, deep link, artifact scope, unavailable services, no blob overfetch và audit.
