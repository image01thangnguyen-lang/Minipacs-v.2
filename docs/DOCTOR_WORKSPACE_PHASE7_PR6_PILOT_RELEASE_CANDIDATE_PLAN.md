# Phase 7 PR6 — Pilot và Release Candidate

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE7_PERFORMANCE_UAT_CONTROLLED_ROLLOUT_PLAN.md](./DOCTOR_WORKSPACE_PHASE7_PERFORMANCE_UAT_CONTROLLED_ROLLOUT_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE7_PR5_FEATURE_FLAGS_COHORT_ROLLBACK_PLAN.md](./DOCTOR_WORKSPACE_PHASE7_PR5_FEATURE_FLAGS_COHORT_ROLLBACK_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Phát hành clinical super-users/pilot hospital, triage phản hồi và chốt release candidate.
...
Trọng tâm riêng: **Entry/exit thresholds, support coverage, Sev policy, feedback ownership, soak và GO/HOLD.**
...
Targeted matrix: Entry/exit thresholds, support coverage, Sev policy, feedback ownership, soak và GO/HOLD.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 7 PR6 — Pilot và Release Candidate.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE7_PERFORMANCE_UAT_CONTROLLED_ROLLOUT_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE7_PR6_PILOT_RELEASE_CANDIDATE_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Phát hành clinical super-users/pilot hospital, triage phản hồi và chốt release candidate.
Trọng tâm: Entry/exit thresholds, support coverage, Sev policy, feedback ownership, soak và GO/HOLD.
