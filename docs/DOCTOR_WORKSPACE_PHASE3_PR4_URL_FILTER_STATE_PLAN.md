# Phase 3 PR4 — URL Filter State, Timezone và Request Cancellation

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE3_WORKLIST_QUERY_FILTER_GRID_PLAN.md](./DOCTOR_WORKSPACE_PHASE3_WORKLIST_QUERY_FILTER_GRID_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE3_PR3_SCOPED_WORKLIST_FACETS_PLAN.md](./DOCTOR_WORKSPACE_PHASE3_PR3_SCOPED_WORKLIST_FACETS_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Chuẩn hóa parser/serializer URL, preset thời gian, debounce và chống response cũ ghi đè bộ lọc mới.
...
- `dashboard/app/components/workspace/WorkspaceSearchBar.tsx`
...
Trọng tâm riêng: **URL round-trip, invalid input, timezone/DST, browser navigation, debounce, Enter-now và race cancellation.**
...
Targeted matrix: URL round-trip, invalid input, timezone/DST, browser navigation, debounce, Enter-now và race cancellation.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 3 PR4 — URL Filter State, Timezone và Request Cancellation.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE3_WORKLIST_QUERY_FILTER_GRID_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE3_PR4_URL_FILTER_STATE_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Chuẩn hóa parser/serializer URL, preset thời gian, debounce và chống response cũ ghi đè bộ lọc mới.
Trọng tâm: URL round-trip, invalid input, timezone/DST, browser navigation, debounce, Enter-now và race cancellation.
