# Phase 4 PR5 — Related Studies Panel và Compare Affordance

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE4_SEVEN_REGION_RELATED_STUDIES_PLAN.md](./DOCTOR_WORKSPACE_PHASE4_SEVEN_REGION_RELATED_STUDIES_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE4_PR4_RELATED_STUDIES_ENDPOINT_PLAN.md](./DOCTOR_WORKSPACE_PHASE4_PR4_RELATED_STUDIES_ENDPOINT_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Hiển thị ca liên quan, range switch, viewer/compare affordance và các trạng thái lỗi/empty an toàn.
...
- `dashboard/app/components/workspace/RelatedStudiesPanel.tsx`
...
Trọng tâm riêng: **Current row, range switch race, keyboard, viewer permission, compare disabled state và partial failure.**
...
Targeted matrix: Current row, range switch race, keyboard, viewer permission, compare disabled state và partial failure.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 4 PR5 — Related Studies Panel và Compare Affordance.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE4_SEVEN_REGION_RELATED_STUDIES_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE4_PR5_RELATED_STUDIES_PANEL_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Hiển thị ca liên quan, range switch, viewer/compare affordance và các trạng thái lỗi/empty an toàn.
Trọng tâm: Current row, range switch race, keyboard, viewer permission, compare disabled state và partial failure.
