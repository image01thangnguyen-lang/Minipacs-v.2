# Phase 6 PR5 — Worklist Orders và Consultations Adoption

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE6_SHARED_LIST_UI_ADOPTION_PLAN.md](./DOCTOR_WORKSPACE_PHASE6_SHARED_LIST_UI_ADOPTION_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE6_PR4_ARCHIVE_SHARED_UI_ADOPTION_PLAN.md](./DOCTOR_WORKSPACE_PHASE6_PR4_ARCHIVE_SHARED_UI_ADOPTION_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Chuyển hai module theo adapters riêng, giữ lifecycle và participant/order authorization.
...
Trọng tâm riêng: **Create/edit/action parity, consultation urgency/lifecycle, scoped rows/counts và deep links.**
...
Targeted matrix: Create/edit/action parity, consultation urgency/lifecycle, scoped rows/counts và deep links.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 6 PR5 — Worklist Orders và Consultations Adoption.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE6_SHARED_LIST_UI_ADOPTION_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE6_PR5_WORKLIST_CONSULTATIONS_ADOPTION_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Chuyển hai module theo adapters riêng, giữ lifecycle và participant/order authorization.
Trọng tâm: Create/edit/action parity, consultation urgency/lifecycle, scoped rows/counts và deep links.
