# Phase 6 PR4 — Archive Shared UI Adoption

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE6_SHARED_LIST_UI_ADOPTION_PLAN.md](./DOCTOR_WORKSPACE_PHASE6_SHARED_LIST_UI_ADOPTION_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE6_PR3_SHARED_DATA_GRID_HARDENING_PLAN.md](./DOCTOR_WORKSPACE_PHASE6_PR3_SHARED_DATA_GRID_HARDENING_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Chuyển Archive sang shared UI mà giữ delivery, print, export, permissions và audit.
...
Trọng tâm riêng: **Filter/row/action parity, scope counts, print/export, feature flag và rollback.**
...
Targeted matrix: Filter/row/action parity, scope counts, print/export, feature flag và rollback.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 6 PR4 — Archive Shared UI Adoption.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE6_SHARED_LIST_UI_ADOPTION_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE6_PR4_ARCHIVE_SHARED_UI_ADOPTION_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Chuyển Archive sang shared UI mà giữ delivery, print, export, permissions và audit.
Trọng tâm: Filter/row/action parity, scope counts, print/export, feature flag và rollback.
