# Phase 5 PR5 — Sign, Approve, Unfinalize, Delivery và HIS Hardening

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE5_REPORT_CONSULTATION_INTEGRATION_PLAN.md](./DOCTOR_WORKSPACE_PHASE5_REPORT_CONSULTATION_INTEGRATION_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE5_PR4_DIRTY_GUARD_CONFLICT_RECOVERY_PLAN.md](./DOCTOR_WORKSPACE_PHASE5_PR4_DIRTY_GUARD_CONFLICT_RECOVERY_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Áp scope + workflow + revision ngay trước mutation, idempotent double-submit và lỗi HIS đúng policy.
...
Trọng tâm riêng: **Stale status, double sign, revoke mid-session, approval separation, HIS retry, transaction/audit.**
...
Targeted matrix: Stale status, double sign, revoke mid-session, approval separation, HIS retry, transaction/audit.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 5 PR5 — Sign, Approve, Unfinalize, Delivery và HIS Hardening.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE5_REPORT_CONSULTATION_INTEGRATION_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE5_PR5_REPORT_ACTION_HARDENING_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Áp scope + workflow + revision ngay trước mutation, idempotent double-submit và lỗi HIS đúng policy.
Trọng tâm: Stale status, double sign, revoke mid-session, approval separation, HIS retry, transaction/audit.
