# Phase 7 PR5 — Feature Flags, Cohort và Rollback Tooling

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE7_PERFORMANCE_UAT_CONTROLLED_ROLLOUT_PLAN.md](./DOCTOR_WORKSPACE_PHASE7_PERFORMANCE_UAT_CONTROLLED_ROLLOUT_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE7_PR4_UAT_FIXTURES_SIGNOFF_PLAN.md](./DOCTOR_WORKSPACE_PHASE7_PR4_UAT_FIXTURES_SIGNOFF_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Xây server-evaluated flags có dependency, cohort audit và rollback từng capability không phá schema/draft.
...
Trọng tâm riêng: **Client tampering, cohort stability, dependency graph, old/new compatibility, rollback drill.**
...
Targeted matrix: Client tampering, cohort stability, dependency graph, old/new compatibility, rollback drill.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 7 PR5 — Feature Flags, Cohort và Rollback Tooling.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE7_PERFORMANCE_UAT_CONTROLLED_ROLLOUT_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE7_PR5_FEATURE_FLAGS_COHORT_ROLLBACK_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Xây server-evaluated flags có dependency, cohort audit và rollback từng capability không phá schema/draft.
Trọng tâm: Client tampering, cohort stability, dependency graph, old/new compatibility, rollback drill.
