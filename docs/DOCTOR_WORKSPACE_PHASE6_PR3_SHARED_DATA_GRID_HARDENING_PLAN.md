# Phase 6 PR3 — Shared DataGrid Hardening và Adapters

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE6_SHARED_LIST_UI_ADOPTION_PLAN.md](./DOCTOR_WORKSPACE_PHASE6_SHARED_LIST_UI_ADOPTION_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE6_PR2_SHARED_FILTER_STATE_PRIMITIVES_PLAN.md](./DOCTOR_WORKSPACE_PHASE6_PR2_SHARED_FILTER_STATE_PRIMITIVES_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Ổn định DataGrid dùng chung và adapter cho các row/action/pagination model hiện hữu.
...
Trọng tâm riêng: **Generic typing, virtualization threshold, pinned columns, preferences namespace, render/query performance.**
...
Targeted matrix: Generic typing, virtualization threshold, pinned columns, preferences namespace, render/query performance.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 6 PR3 — Shared DataGrid Hardening và Adapters.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE6_SHARED_LIST_UI_ADOPTION_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE6_PR3_SHARED_DATA_GRID_HARDENING_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Ổn định DataGrid dùng chung và adapter cho các row/action/pagination model hiện hữu.
Trọng tâm: Generic typing, virtualization threshold, pinned columns, preferences namespace, render/query performance.
