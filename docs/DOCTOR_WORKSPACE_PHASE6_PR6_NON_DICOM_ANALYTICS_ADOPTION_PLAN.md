# Phase 6 PR6 — Non-DICOM và Analytics Drilldown Adoption

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE6_SHARED_LIST_UI_ADOPTION_PLAN.md](./DOCTOR_WORKSPACE_PHASE6_SHARED_LIST_UI_ADOPTION_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE6_PR5_WORKLIST_CONSULTATIONS_ADOPTION_PLAN.md](./DOCTOR_WORKSPACE_PHASE6_PR5_WORKLIST_CONSULTATIONS_ADOPTION_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Chuẩn hóa Non-DICOM list cùng statistics/command-center drilldowns, không ép chart vào grid.
...
Trọng tâm riêng: **Upload/media lifecycle, facility scope, aggregate-to-drilldown parity, bundle split và errors.**
...
Targeted matrix: Upload/media lifecycle, facility scope, aggregate-to-drilldown parity, bundle split và errors.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 6 PR6 — Non-DICOM và Analytics Drilldown Adoption.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE6_SHARED_LIST_UI_ADOPTION_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE6_PR6_NON_DICOM_ANALYTICS_ADOPTION_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Chuẩn hóa Non-DICOM list cùng statistics/command-center drilldowns, không ép chart vào grid.
Trọng tâm: Upload/media lifecycle, facility scope, aggregate-to-drilldown parity, bundle split và errors.
