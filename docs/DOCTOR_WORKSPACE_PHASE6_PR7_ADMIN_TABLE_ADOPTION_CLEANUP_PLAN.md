# Phase 6 PR7 — Admin Table Adoption và Legacy Cleanup

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE6_SHARED_LIST_UI_ADOPTION_PLAN.md](./DOCTOR_WORKSPACE_PHASE6_SHARED_LIST_UI_ADOPTION_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE6_PR6_NON_DICOM_ANALYTICS_ADOPTION_PLAN.md](./DOCTOR_WORKSPACE_PHASE6_PR6_NON_DICOM_ANALYTICS_ADOPTION_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Migrate bảng admin theo wave, giữ destructive confirmation/audit rồi xóa component hết consumer.
...
Trọng tâm riêng: **Admin permissions, reason dialogs, bulk actions, dead-code proof, CSS/token audit và rollback.**
...
Targeted matrix: Admin permissions, reason dialogs, bulk actions, dead-code proof, CSS/token audit và rollback.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 6 PR7 — Admin Table Adoption và Legacy Cleanup.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE6_SHARED_LIST_UI_ADOPTION_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE6_PR7_ADMIN_TABLE_ADOPTION_CLEANUP_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Migrate bảng admin theo wave, giữ destructive confirmation/audit rồi xóa component hết consumer.
Trọng tâm: Admin permissions, reason dialogs, bulk actions, dead-code proof, CSS/token audit và rollback.
