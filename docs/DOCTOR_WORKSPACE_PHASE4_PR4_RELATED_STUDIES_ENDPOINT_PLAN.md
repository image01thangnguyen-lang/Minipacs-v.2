# Phase 4 PR4 — Related Studies Endpoint

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE4_SEVEN_REGION_RELATED_STUDIES_PLAN.md](./DOCTOR_WORKSPACE_PHASE4_SEVEN_REGION_RELATED_STUDIES_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE4_PR3_SELECTION_DEEP_LINK_RACE_PLAN.md](./DOCTOR_WORKSPACE_PHASE4_PR3_SELECTION_DEEP_LINK_RACE_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Query lịch sử bệnh nhân độc lập, authorize anchor và lọc scope trước count/page mà không tin patient ID client.
...
- `dashboard/app/actions/related-studies-actions.ts`
- `dashboard/lib/workspace/related-studies.ts`
...
Trọng tâm riêng: **Anchor authorization, identity collision, range/cursor, cross-hospital leak, no fuzzy name match và no-N+1.**
...
Targeted matrix: Anchor authorization, identity collision, range/cursor, cross-hospital leak, no fuzzy name match và no-N+1.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 4 PR4 — Related Studies Endpoint.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE4_SEVEN_REGION_RELATED_STUDIES_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE4_PR4_RELATED_STUDIES_ENDPOINT_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Query lịch sử bệnh nhân độc lập, authorize anchor và lọc scope trước count/page mà không tin patient ID client.
Trọng tâm: Anchor authorization, identity collision, range/cursor, cross-hospital leak, no fuzzy name match và no-N+1.
