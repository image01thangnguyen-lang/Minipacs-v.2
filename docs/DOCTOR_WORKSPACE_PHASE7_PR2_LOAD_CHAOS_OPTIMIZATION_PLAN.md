# Phase 7 PR2 — Load, Chaos và Query Optimization

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE7_PERFORMANCE_UAT_CONTROLLED_ROLLOUT_PLAN.md](./DOCTOR_WORKSPACE_PHASE7_PERFORMANCE_UAT_CONTROLLED_ROLLOUT_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE7_PR1_OBSERVABILITY_SLO_PLAN.md](./DOCTOR_WORKSPACE_PHASE7_PR1_OBSERVABILITY_SLO_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Đo realistic concurrency/cardinality/network và tối ưu có bằng chứng khi Orthanc/HIS/DB suy giảm.
...
- `load fixtures/scripts`
- `Prisma queries/indexes`
- `deployment limits`
...
Trọng tâm riêng: **P50/P95/P99, pool saturation, no-N+1, packet loss, outage/recovery và regression budget.**
...
Targeted matrix: P50/P95/P99, pool saturation, no-N+1, packet loss, outage/recovery và regression budget.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 7 PR2 — Load, Chaos và Query Optimization.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE7_PERFORMANCE_UAT_CONTROLLED_ROLLOUT_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE7_PR2_LOAD_CHAOS_OPTIMIZATION_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Đo realistic concurrency/cardinality/network và tối ưu có bằng chứng khi Orthanc/HIS/DB suy giảm.
Trọng tâm: P50/P95/P99, pool saturation, no-N+1, packet loss, outage/recovery và regression budget.
