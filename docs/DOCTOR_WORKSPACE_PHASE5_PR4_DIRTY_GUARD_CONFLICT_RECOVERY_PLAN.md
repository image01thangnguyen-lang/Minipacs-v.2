# Phase 5 PR4 — Dirty Guard và Conflict Recovery

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE5_REPORT_CONSULTATION_INTEGRATION_PLAN.md](./DOCTOR_WORKSPACE_PHASE5_REPORT_CONSULTATION_INTEGRATION_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE5_PR3_REVISIONED_AUTOSAVE_PLAN.md](./DOCTOR_WORKSPACE_PHASE5_PR3_REVISIONED_AUTOSAVE_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Chặn đổi ca/rời trang khi dirty và cung cấp lưu–bỏ–ở lại cùng xử lý concurrent edit không overwrite.
...
- `dashboard/app/components/workspace/DoctorWorkspace.tsx`
- `dashboard/app/components/workspace/ReportWorkspacePanel.tsx`
...
Trọng tâm riêng: **All navigation paths, A→B cancel, browser unload fallback, two-session conflict và unsaved-copy recovery.**
...
Targeted matrix: All navigation paths, A→B cancel, browser unload fallback, two-session conflict và unsaved-copy recovery.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 5 PR4 — Dirty Guard và Conflict Recovery.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE5_REPORT_CONSULTATION_INTEGRATION_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE5_PR4_DIRTY_GUARD_CONFLICT_RECOVERY_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Chặn đổi ca/rời trang khi dirty và cung cấp lưu–bỏ–ở lại cùng xử lý concurrent edit không overwrite.
Trọng tâm: All navigation paths, A→B cancel, browser unload fallback, two-session conflict và unsaved-copy recovery.
