# Phase 6 PR2 — Shared Filter, State và Status Primitives

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE6_SHARED_LIST_UI_ADOPTION_PLAN.md](./DOCTOR_WORKSPACE_PHASE6_SHARED_LIST_UI_ADOPTION_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE6_PR1_SHARED_UI_CONTRACT_INVENTORY_PLAN.md](./DOCTOR_WORKSPACE_PHASE6_PR1_SHARED_UI_CONTRACT_INVENTORY_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Trích xuất header/filter/date/facet/status/empty/error/access-denied primitives không chứa policy module.
...
Trọng tâm riêng: **Controlled APIs, URL adapter, domain status registry, keyboard/ARIA và no hardcoded palette.**
...
Targeted matrix: Controlled APIs, URL adapter, domain status registry, keyboard/ARIA và no hardcoded palette.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 6 PR2 — Shared Filter, State và Status Primitives.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE6_SHARED_LIST_UI_ADOPTION_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE6_PR2_SHARED_FILTER_STATE_PRIMITIVES_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Trích xuất header/filter/date/facet/status/empty/error/access-denied primitives không chứa policy module.
Trọng tâm: Controlled APIs, URL adapter, domain status registry, keyboard/ARIA và no hardcoded palette.
