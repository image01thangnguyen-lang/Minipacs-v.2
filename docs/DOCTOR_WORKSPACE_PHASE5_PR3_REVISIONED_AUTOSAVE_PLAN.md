# Phase 5 PR3 — Revisioned Draft Autosave

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE5_REPORT_CONSULTATION_INTEGRATION_PLAN.md](./DOCTOR_WORKSPACE_PHASE5_REPORT_CONSULTATION_INTEGRATION_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE5_PR2_REPORT_PANEL_EXTRACTION_PLAN.md](./DOCTOR_WORKSPACE_PHASE5_PR2_REPORT_PANEL_EXTRACTION_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Bổ sung autosave có revision/idempotency, trạng thái SAVING/SAVED/ERROR và không báo lưu giả.
...
Trọng tâm riêng: **Rapid typing, in-flight ordering, retry, stale revision, duplicate request, session expiry và audit scrub.**
...
Targeted matrix: Rapid typing, in-flight ordering, retry, stale revision, duplicate request, session expiry và audit scrub.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 5 PR3 — Revisioned Draft Autosave.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE5_REPORT_CONSULTATION_INTEGRATION_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE5_PR3_REVISIONED_AUTOSAVE_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Bổ sung autosave có revision/idempotency, trạng thái SAVING/SAVED/ERROR và không báo lưu giả.
Trọng tâm: Rapid typing, in-flight ordering, retry, stale revision, duplicate request, session expiry và audit scrub.
