# Phase 3 PR2 — Scoped Worklist Query

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE3_WORKLIST_QUERY_FILTER_GRID_PLAN.md](./DOCTOR_WORKSPACE_PHASE3_WORKLIST_QUERY_FILTER_GRID_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE3_PR1_WORKLIST_CONTRACT_INDEX_BASELINE_PLAN.md](./DOCTOR_WORKSPACE_PHASE3_PR1_WORKLIST_CONTRACT_INDEX_BASELINE_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Xây RIS worklist query đã áp scope trước pagination, stable cursor và row mapper không gọi Orthanc theo từng ca.
...
- `dashboard/app/db.ts`
- `dashboard/lib/authz/scope/scope-filter-builder.ts`
- `dashboard/lib/worklist/*`
...
Trọng tâm riêng: **Scope predicate, stable sort, cursor tampering, batch relations, allowedActions và no-N+1.**
...
Targeted matrix: Scope predicate, stable sort, cursor tampering, batch relations, allowedActions và no-N+1.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 3 PR2 — Scoped Worklist Query.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE3_WORKLIST_QUERY_FILTER_GRID_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE3_PR2_SCOPED_WORKLIST_QUERY_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Xây RIS worklist query đã áp scope trước pagination, stable cursor và row mapper không gọi Orthanc theo từng ca.
Trọng tâm: Scope predicate, stable sort, cursor tampering, batch relations, allowedActions và no-N+1.
