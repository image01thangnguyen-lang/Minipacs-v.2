# Phase 3 PR6 — Column Preferences, Density và Allowed Actions

Ngày lập: 2026-07-11  
Phase cha: [DOCTOR_WORKSPACE_PHASE3_WORKLIST_QUERY_FILTER_GRID_PLAN.md](./DOCTOR_WORKSPACE_PHASE3_WORKLIST_QUERY_FILTER_GRID_PLAN.md), sau [DOCTOR_WORKSPACE_PHASE3_PR5_STUDY_DATA_GRID_FOUNDATION_PLAN.md](./DOCTOR_WORKSPACE_PHASE3_PR5_STUDY_DATA_GRID_FOUNDATION_PLAN.md)  
Trạng thái: kế hoạch triển khai độc lập; inventory code thực tế là gate bắt buộc

## 1. Mục tiêu và kết quả

Lưu preference theo user và render tác vụ từ allowedActions nhưng vẫn giữ server action là quyết định cuối.
...
Trọng tâm riêng: **Schema version, allowlist/clamp, cross-user isolation, reset, forged actions và read-only rendering.**
...
Targeted matrix: Schema version, allowlist/clamp, cross-user isolation, reset, forged actions và read-only rendering.
...
```text
Làm việc tại C:\Antigravity\Minipacs-v.2. Triển khai Phase 3 PR6 — Column Preferences, Density và Allowed Actions.

Đọc master plan, docs/DOCTOR_WORKSPACE_PHASE3_WORKLIST_QUERY_FILTER_GRID_PLAN.md, docs/DOCTOR_WORKSPACE_PHASE3_PR6_COLUMN_PREFERENCES_ALLOWED_ACTIONS_PLAN.md và mọi PR tiền đề. Trước khi sửa, chạy git status/diff read-only; inventory callers/schema/authz scope/workflow/audit/tests/flags; bảo toàn worktree và ghi baseline kiểm chứng được.

Mục tiêu: Lưu preference theo user và render tác vụ từ allowedActions nhưng vẫn giữ server action là quyết định cuối.
Trọng tâm: Schema version, allowlist/clamp, cross-user isolation, reset, forged actions và read-only rendering.
