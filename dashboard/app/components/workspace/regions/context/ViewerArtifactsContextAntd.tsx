...
export function ViewerArtifactsContextAntd({ studyInstanceUid }: ViewerArtifactsContextAntdProps) {
  ...
  if (loading) {
    return <div className="mt-4 p-3 text-sm text-gray-500 text-center border border-[#303030] rounded-md bg-[#1F1F1F]"><Spin /> Đang tải hình ảnh...</div>;
  }
            ...
            <div className="grid grid-cols-2 gap-2">
              {artifacts.keyImages.slice(0, 4).map((img, i) => (
                <div key={img.id || i} className="aspect-square bg-[#141414] border border-[#303030] rounded overflow-hidden flex items-center justify-center relative group cursor-pointer hover:border-[#13C2C2] transition-colors">
                  <span className="text-sm text-gray-500 group-hover:text-cyan-400">Key Image</span>
                </div>
              ))}
              {artifacts.keyImages.length > 4 && (
                <div className="text-sm text-gray-500 text-center col-span-2 pt-1">
          ...
          <div className="text-sm">
            ...
            <div className="flex flex-col gap-1.5 max-h-[120px] overflow-y-auto pr-1">
              {artifacts.measurements.map(m => (
                <div key={m.id} className="text-sm bg-[#141414] p-1.5 border border-[#303030] rounded truncate text-gray-400">
