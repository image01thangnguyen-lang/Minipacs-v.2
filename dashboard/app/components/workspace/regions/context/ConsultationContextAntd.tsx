    ...
    <section aria-label="Consultations" className="mt-4 border border-[#303030] rounded-md bg-[#1F1F1F]">
      ...
      <div className="p-3 flex flex-col gap-2">
        {loading ? (
          ...
          consultations.map(c => (
            <div key={c.id} className="text-[11px] bg-[#141414] border border-[#303030] p-2 rounded">
              <div className="flex justify-between items-start mb-1">
                <Text className="font-semibold text-gray-300 truncate" style={{ maxWidth: '140px' }}>{c.title}</Text>
                <Tag color={c.status === 'FINISHED' ? 'success' : 'processing'} className="text-[9px] m-0 border-0 bg-opacity-20 font-medium">
                  {c.status}
                </Tag>
              </div>
