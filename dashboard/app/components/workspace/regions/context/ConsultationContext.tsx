    ...
    <section aria-label="Consultations" className="mt-4 border border-vin-border rounded-md bg-vin-shell/30">
      <div className="flex items-center justify-between border-b border-vin-border bg-vin-shell px-3 py-2">
        ...
        {canCreate && (
          <button
            ...
            className="text-sm flex items-center gap-1 font-medium text-vin-primary hover:text-vin-primary-hover"
      ...
      <div className="p-3 flex flex-col gap-2">
        {loading ? (
          ...
          <div className="text-sm text-vin-muted text-center py-2">Không có hội chẩn</div>
        ) : (
          consultations.map(c => (
            <div key={c.id} className="text-sm bg-vin-panel border border-vin-border p-2 rounded">
