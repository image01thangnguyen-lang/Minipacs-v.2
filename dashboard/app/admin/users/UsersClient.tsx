  ...
  return (
    <section className="rounded border border-vin-border bg-vin-shell px-3 py-3">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-sm font-bold uppercase tracking-wide text-vin-muted">Quyền của vai trò</div>
          <div className="mt-1 text-sm font-bold text-white">{title}</div>
          {description && <div className="mt-0.5 text-sm leading-relaxed text-vin-muted">{description}</div>}
        </div>
        ...
        {permissionGroups.map(group => {
          ...
          return (
            <div key={group.title} className="rounded border border-vin-border/70 bg-vin-panel px-2.5 py-2">
              <div className="mb-1.5 text-sm font-bold uppercase tracking-wide text-vin-muted">{group.title}</div>
              <div className="grid grid-cols-1 gap-1.5">
                {active.map(permission => (
                  <div key={permission} className="flex items-center gap-1.5 text-sm text-vin-text2">
    ...
    <div className="grid grid-cols-1 gap-3">
      {permissionGroups.map(group => (
        <section key={group.title} className="rounded border border-vin-border bg-vin-shell p-3">
          <div className="mb-2 text-sm font-bold uppercase tracking-wide text-vin-muted">{group.title}</div>
          <div className="grid grid-cols-1 gap-2">
            {group.permissions.map(permission => (
              <label key={permission} className="flex items-center gap-2 text-sm text-vin-text2">
                <input
                  name="permissions"
                  type="checkbox"
                  value={permission}
...
function PermissionMatrix({ roleProfiles }: { roleProfiles: RoleProfile[] }) {
  ...
  return (
    <section className="rounded border border-vin-border bg-vin-shell p-3 text-left">
      <div className="mb-2 text-sm font-bold uppercase tracking-wide text-vin-muted">Ma trận quyền</div>
...
export default function UsersClient({ useAntd }: { useAntd?: boolean }) {
  ...
  if (useAntd) {
    return (
      <UsersAntd
        ...
        users={users} roleProfiles={roleProfiles}
        ...
        createFormKey={createFormKey} setCreateFormKey={setCreateFormKey}
        createRoleFormKey={createRoleFormKey} setCreateRoleFormKey={setCreateRoleFormKey}
        createRoleProfileId={createRoleProfileId} setCreateRoleProfileId={setCreateRoleProfileId}
        editRoleProfileId={editRoleProfileId} setEditRoleProfileId={setEditRoleProfileId}
        importResults={importResults} setImportResults={setImportResults}
        filteredUsers={filteredUsers} filteredRoles={filteredRoles}
        ...
        defaultRoleProfileId={defaultRoleProfileId}
        selectedCreateRoleProfile={selectedCreateRoleProfile}
        selectedEditRoleProfile={selectedEditRoleProfile}
        selectedUser={selectedUser}
        selectedRole={selectedRole}
        createUserIsDoctor={createUserIsDoctor}
        editUserIsDoctor={editUserIsDoctor}
      />
    );
  }
          ...
          <div className="mb-2 flex items-center justify-between gap-3">
            ...
            <div className="flex gap-2">
              <button
                ...
                className="flex items-center gap-1.5 rounded border border-vin-border bg-vin-panel px-2.5 py-1 text-sm font-semibold text-vin-muted transition hover:text-white hover:bg-vin-panel2"
              >
        ...
        {showUserCreate ? (
          <React.Fragment key={createFormKey}>
            ...
            <form onSubmit={handleCreateUserSubmit} className="min-h-0 flex-1 space-y-4 overflow-auto p-4 scr-dark" encType="multipart/form-data" autoComplete="off">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="mb-1 block text-sm font-bold uppercase tracking-wide text-vin-muted">Username</label>
                ...
                <div>
                  <label className="mb-1 block text-sm font-bold uppercase tracking-wide text-vin-muted">Họ tên</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="mb-1 block text-sm font-bold uppercase tracking-wide text-vin-muted">Mật khẩu</label>
                ...
                <div>
                  <label className="mb-1 block text-sm font-bold uppercase tracking-wide text-vin-muted">Vai trò</label>
                  <CustomSelect
                    name="roleProfileId"
              ...
              <div className="border-t border-vin-border/50 pt-4">
                <p className="mb-3 text-sm font-bold uppercase tracking-wide text-vin-muted">Hồ sơ bác sĩ</p>
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="mb-1 block text-sm font-bold uppercase tracking-wide text-vin-muted">Chức danh</label>
                  ...
                  <div>
                    <label className="mb-1 block text-sm font-bold uppercase tracking-wide text-vin-muted">Chuyên khoa</label>
                ...
                <div className="mt-3">
                  <label className="mb-1 block text-sm font-bold uppercase tracking-wide text-vin-muted">Số CCHN</label>
                ...
                <div className="mt-3">
                  <label className="mb-1 flex items-center gap-1 text-sm font-bold uppercase tracking-wide text-vin-muted">
                    <Upload className="h-3 w-3" />
                    Chữ ký scan
                  </label>
                  <input name="signature" type="file" accept="image/jpeg,image/png,image/webp,image/gif" disabled={!createUserIsDoctor} className="block w-full text-sm text-vin-muted file:mr-2 file:rounded file:border-0 file:bg-vin-shell file:px-3 file:py-2 file:text-sm file:font-semibold file:text-vin-text2 hover:file:text-white disabled:opacity-45" />
                </div>
              </div>
          ...
          <React.Fragment key={createRoleFormKey}>
            ...
            <form onSubmit={handleCreateRoleSubmit} className="min-h-0 flex-1 space-y-4 overflow-auto p-4 scr-dark">
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="mb-1 block text-sm font-bold uppercase tracking-wide text-vin-muted">Tên vai trò</label>
                ...
                <div>
                  <label className="mb-1 block text-sm font-bold uppercase tracking-wide text-vin-muted">Mã vai trò</label>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="mb-1 block text-sm font-bold uppercase tracking-wide text-vin-muted">Nhóm gốc</label>
                  <CustomSelect
                    name="baseRole"
          ...
          <React.Fragment>
            ...
            <div className="min-h-0 flex-1 overflow-auto p-4 flex flex-col gap-4 scr-dark">
              <form onSubmit={handleImportSubmit} className="space-y-4">
                <div>
                  <label className="mb-1 block text-sm font-bold uppercase tracking-wide text-vin-muted">File CSV</label>
                  <input name="file" type="file" accept=".csv" required className="block w-full text-sm text-vin-muted file:mr-2 file:rounded file:border-0 file:bg-vin-shell file:px-3 file:py-2 file:text-sm file:font-semibold file:text-vin-text2 hover:file:text-white" />
                  <p className="mt-1 text-sm text-vin-muted">Cấu trúc: username, fullname, role (Mã role hoặc Tên role). Cần có dòng tiêu đề.</p>
                </div>
            ...
            <form key={selectedUser.id} onSubmit={handleUpdateUserSubmit} className="min-h-0 flex-1 space-y-4 overflow-auto p-4 scr-dark" encType="multipart/form-data">
              ...
              <div className="border-t border-vin-border/50 pt-4">
                ...
                <div className="mt-3">
                  ...
                  <input name="signature" type="file" accept="image/jpeg,image/png,image/webp,image/gif" disabled={!editUserIsDoctor} className="block w-full text-sm text-vin-muted file:mr-2 file:rounded file:border-0 file:bg-vin-shell file:px-3 file:py-2 file:text-sm file:font-semibold file:text-vin-text2 hover:file:text-white disabled:opacity-45" />
            ...
            <div className="flex-none border-b border-vin-border bg-vin-panel2 px-4 py-3">
              <div className="mb-2 flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="mt-1 font-mono text-sm text-vin-muted">{selectedRole.code}</p>
                </div>
          ...
          <div className="flex h-full flex-col items-center justify-center px-6 text-center">
            ...
            <p className="max-w-[280px] text-sm leading-relaxed text-vin-muted">
              {activeTab === "users"
