...
import CatalogsAntd from "./CatalogsAntd";

export default function CatalogsClient({ useAntd }: { useAntd?: boolean }) {
  ...
  const enableSharedUI = process.env.NEXT_PUBLIC_ENABLE_ADMIN_CATALOGS_SHARED_UI === "true" || useAntd;
  ...
  const handleCreateSubmit = async (eventOrFormData: React.FormEvent<HTMLFormElement> | FormData) => {
    let formData: FormData;
    if (eventOrFormData instanceof FormData) {
      formData = eventOrFormData;
    } else {
      eventOrFormData.preventDefault();
      formData = new FormData(eventOrFormData.currentTarget);
    }
    ...
    try {
      ...
      setMode("view");
      if (!(eventOrFormData instanceof FormData)) {
        eventOrFormData.currentTarget.reset();
      }
    } catch (error) {
      console.error(error);
  ...
  const handleUpdateSubmit = async (eventOrFormData: React.FormEvent<HTMLFormElement> | FormData) => {
    let formData: FormData;
    if (eventOrFormData instanceof FormData) {
      formData = eventOrFormData;
    } else {
