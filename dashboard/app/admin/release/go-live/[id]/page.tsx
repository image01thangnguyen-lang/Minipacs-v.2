      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <div className="space-y-6">
          ...
          <section className="rounded-md border border-slate-700 bg-slate-900/40 p-5">
            ...
            <div className="mt-4 space-y-3">{release.knownIssues.map(issue => <div key={issue.id} className="rounded-md border border-slate-700 p-3 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><strong>{issue.riskLevel}: {issue.description}</strong><span className="rounded bg-slate-800 px-2 py-0.5 text-sm">{issue.status}</span></div>{issue.workaround && <p className="mt-2 text-slate-400">Workaround: {issue.workaround}</p>}{issue.acceptanceNotes && <p className="mt-2 text-sm text-slate-400">Acceptance: {issue.acceptanceNotes} · {issue.acceptedByUser?.fullName || "-"}</p>}{canManage && <form action={async formData => { "use server"; await updateKnownIssueStatus(issue.id, String(formData.get("status") || ""), String(formData.get("notes") || "")); }} className="mt-3 grid gap-2 sm:grid-cols-[130px_1fr_auto]"><select name="status" defaultValue={issue.status} className="h-8 rounded-md border border-slate-600 bg-slate-950 px-2 text-sm"><option>OPEN</option><option>ACCEPTED</option><option>RESOLVED</option></select><input name="notes" defaultValue={issue.acceptanceNotes || ""} placeholder="Ghi chú bắt buộc khi ACCEPTED" className="h-8 rounded-md border border-slate-600 bg-slate-950 px-2 text-sm" /><button className="h-8 rounded-md border border-slate-600 px-3 text-sm">Cập nhật</button></form>}</div>)}{!release.knownIssues.length && <p className="text-sm text-slate-500">Không có known issue.</p>}</div>
