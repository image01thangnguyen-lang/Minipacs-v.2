  ...
  return (
    <main className="mx-auto flex w-full max-w-5xl flex-col gap-6 p-6">
      ...
      <section className="rounded-md border border-slate-700 bg-slate-900/40 p-5">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2"><div className="rounded border border-slate-700 p-3 text-sm"><span className="text-slate-400">Prepared by</span><strong className="mt-1 block">{handoff?.preparedByUser?.fullName || "-"}</strong><p className="text-sm text-slate-500">{handoff?.preparedAt?.toLocaleString("vi-VN") || ""}</p></div><div className="rounded border border-slate-700 p-3 text-sm"><span className="text-slate-400">Accepted by</span><strong className="mt-1 block">{handoff?.acceptedByUser?.fullName || "-"}</strong><p className="text-sm text-slate-500">{handoff?.acceptedAt?.toLocaleString("vi-VN") || ""}</p></div></div>
