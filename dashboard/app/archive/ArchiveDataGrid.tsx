          ...
          <>
            ...
            {(row.hisSyncStatus || row.hisResultStatus) && (
              <div className="mt-1 flex flex-col gap-0.5 text-[9px] font-semibold">
                {row.hisSyncStatus && (
                  <Link href={`/admin/his`} onClick={(e) => e.stopPropagation()} className={`mt-1 block w-fit rounded px-1.5 py-0.5 text-[9px] font-semibold transition hover:opacity-80 ${row.hisSyncStatus === 'FAILED' ? 'bg-red-900/40 text-red-300' : 'bg-emerald-900/40 text-emerald-300'}`} title="Click to view HIS Logs">
