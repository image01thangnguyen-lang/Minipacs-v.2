...
function makeTree(
  units: { id: string; type: string; parentId: string | null }[],
): OrganizationTree {
...
function makePrincipal(
  overrides: Partial<ScopePrincipal> = {},
): ScopePrincipal {
  return {
    userId: "user1",
    baseRole: "DOCTOR",
...
async function main() {
  ...
  await run("returns MISSING_IDENTIFIER for null aeTitle", async () => {
    ...
    assert(
      ...
      `Expected MISSING_IDENTIFIER, got ${result.status}`,
    );
  ...
  await run("returns MATCHED for exactly one active node", async () => {
    ...
    assert(
      ...
      `Expected MATCHED, got ${result.status}`,
    );
  ...
  await run("returns AMBIGUOUS for multiple active nodes", async () => {
    ...
    assert(
      ...
      `Expected AMBIGUOUS, got ${result.status}`,
    );
    assert(
      result.dicomNodeId === null,
      "AMBIGUOUS should have null dicomNodeId",
    );
  });
  ...
  await run("returns NO_MATCH when no nodes found", async () => {
    ...
    assert(
      ...
      `Expected NO_MATCH, got ${result.status}`,
    );
  });

  await run(
    "returns RESOURCE_INACTIVE when only inactive nodes found",
    async () => {
      const deps: AeTitleResolverDeps = {
        findDicomNodesByAeTitle: async () => [
          { id: "node1", aeTitle: "CT1", isActive: false, facilityId: "hosp1" },
        ],
      };
      const result = await resolveAeTitle("CT1", deps);
      assert(
        result.status === "RESOURCE_INACTIVE",
        `Expected RESOURCE_INACTIVE, got ${result.status}`,
      );
    },
  );
  ...
  await run("caches results in ScopeRequestContext", async () => {
    ...
    const deps: AeTitleResolverDeps = {
      findDicomNodesByAeTitle: async () => {
        callCount++;
        return [{ id: "n1", aeTitle: "CT1", isActive: true, facilityId: "h1" }];
      },
    };
  ...
  await run("uses performingUnitId when available", async () => {
    ...
    assert(
      result.ancestorUnitIds.includes("hosp1"),
      "Should include hosp1 as ancestor",
    );
    assert(
      result.ancestorUnitIds.includes("chain1"),
      "Should include chain1 as ancestor",
    );
  });

  await run(
    "falls back to stationAeTitle when no performingUnitId",
    async () => {
      const deps: ResourceContextDeps = {
        findDicomNodesByAeTitle: async () => [
          { id: "node1", aeTitle: "CT1", isActive: true, facilityId: "room1" },
        ],
        findDicomNodeById: async () => null,
  ...
  await run(
    "uses machineId for NonDicom when no performingUnitId",
    async () => {
      const deps: ResourceContextDeps = {
        findDicomNodesByAeTitle: async () => [],
        findDicomNodeById: async (id) =>
          id === "machine1"
            ? {
                id: "machine1",
                facilityId: "dept1",
  ...
  await run(
    "facilityId valid + machineId inactive => classified=false and RESOURCE_INACTIVE",
    async () => {
      const deps: ResourceContextDeps = {
        findDicomNodesByAeTitle: async () => [],
        findDicomNodeById: async (id) =>
      ...
      assert(
        ...
        "Should be unclassified due to inactive machine",
      );
      assert(
        result.aeResolution.status === "RESOURCE_INACTIVE",
        "Status should be RESOURCE_INACTIVE",
      );
    },
  );

  await run(
    "machineId inactive + stationAeTitle active => deny RESOURCE_INACTIVE",
    async () => {
      const deps: ResourceContextDeps = {
        findDicomNodesByAeTitle: async (ae) =>
          ae === "CT1"
            ? [
                {
                  id: "node2",
      ...
      assert(
        result.aeResolution.status === "RESOURCE_INACTIVE",
        "Status should be RESOURCE_INACTIVE and not overwritten by AE resolver",
      );
    },
  );
  ...
  await run("loads principal with correct permissions", async () => {
    ...
    assert(
      p!.globalPermissions.includes("studies.read"),
      "Should have studies.read",
    );
  });
  ...
  await run("uses roleProfile permissions when active", async () => {
    const deps: PrincipalLoaderDeps = {
      findUserById: async () => ({
        ...
        roleProfile: {
          id: "rp1",
          permissions: ["studies.read", "reports.read"],
          isActive: true,
          baseRole: "DOCTOR",
        },
      }),
    ...
    assert(p !== null, "Principal should not be null");
    assert(
      p!.globalPermissions.length === 2,
      `Expected 2 permissions, got ${p!.globalPermissions.length}`,
    );
  });
  ...
  await run("ALLOW grant at resource's facility → allowed", async () => {
    ...
    const resourceCtx = {
      performingUnitId: "dept1",
      ancestorUnitIds: ["hosp1", "chain1"],
      dicomNodeId: null,
      facilityUnitId: "dept1",
      aeResolution: {
        status: "MISSING_IDENTIFIER" as const,
        dicomNodeId: null,
        facilityUnitId: null,
        aeTitle: null,
      },
  ...
  await run("DENY always wins over ALLOW", async () => {
    ...
    assert(
      result.traces.some((t) => t.effect === "DENY"),
      "Should have DENY trace",
    );
  });

  await run(
    "Grant at parent with includeDescendants applies to child",
    async () => {
      const deps: GrantRepositoryDeps = {
        findGrantsByUserAndCapability: async () => [
          makeGrant({
      ...
      const resourceCtx = {
        performingUnitId: "room1",
        ancestorUnitIds: ["dept1", "hosp1", "chain1"],
        dicomNodeId: null,
        facilityUnitId: "room1",
        aeResolution: {
          status: "MISSING_IDENTIFIER" as const,
          dicomNodeId: null,
          facilityUnitId: null,
      ...
      assert(
        result.traces[0].matchType === "INHERITED",
        "Match should be INHERITED",
      );
    },
  );

  await run(
    "Grant at parent without includeDescendants does NOT apply to child",
    async () => {
      const deps: GrantRepositoryDeps = {
        findGrantsByUserAndCapability: async () => [
          makeGrant({
      ...
      assert(
        ...
        "Should not inherit without includeDescendants",
      );
    },
  );

  await run(
    "DENY at child blocks resource in that subtree even with parent ALLOW",
    async () => {
      const deps: GrantRepositoryDeps = {
        findGrantsByUserAndCapability: async () => [
          makeGrant({
  ...
  await run(
    "User grant combined with roleProfile grant — DENY from roleProfile wins",
    async () => {
      const deps: GrantRepositoryDeps = {
        findGrantsByUserAndCapability: async () => [
          makeGrant({
  ...
  await run("Grant with inactive facilityUnit is skipped", async () => {
    ...
    assert(
      result.allowed === false,
      "Grant with inactive facility should be skipped",
    );
  });
  ...
  await run("No row = no opinion (not DENY)", async () => {
    ...
    const result = await evaluateLegacyPermissions(
      makePrincipal(),
      "READ_STUDY",
      resourceCtx,
      deps,
    ...
    assert(
      result.hasMatchingOpinion === false,
      "No row should mean no opinion",
    );
    assert(
      result.allowed === false,
      "allowed should be false (no opinion default)",
    );
  });
  ...
  await run(
    "Non-DICOM capability returns no opinion (no legacy mapping)",
    async () => {
      const deps: LegacyAdapterDeps = {
        findLegacyPermissions: async () => [],
      };
      ...
      assert(
        result.hasMatchingOpinion === false,
        "Non-DICOM should have no legacy opinion",
      );
    },
  );
  ...
  await run("Legacy perms cached in request context", async () => {
    ...
    const ctx = ScopeRequestContext.create();
    await evaluateLegacyPermissions(
      makePrincipal(),
      "READ_STUDY",
      resourceCtx,
      deps,
  ...
  function makeScopeResolverDeps(
    overrides: Partial<ScopeResolverDeps> = {},
  ): ScopeResolverDeps {
    return {
      findUserById: async () => ({
        id: "user1",
  ...
  await run(
    "OFF mode: effectiveAllowed = baselineAllowed regardless of scope",
    async () => {
      ...
      assert(
        result.effectiveAllowed === true,
        "OFF: baseline has studies.read, so effectiveAllowed should be true",
      );
  ...
  await run(
    "SHADOW mode: effectiveAllowed = baselineAllowed, logs divergence",
    async () => {
      const originalMode = process.env.AUTHORIZATION_MODE;
      ...
      assert(
        result.effectiveAllowed === true,
        "SHADOW: effectiveAllowed should still be baselineAllowed=true",
      );
      assert(
        result.proposedAllowed === false,
        "proposed should be false (DENY grant)",
      );
      assert(
        warnMessages.some((m) => m.includes("SHADOW divergence")),
        "Should log divergence",
      );
  ...
  await run(
    "ENFORCE mode: effectiveAllowed = baselineAllowed && proposedAllowed",
    async () => {
      const originalMode = process.env.AUTHORIZATION_MODE;
      ...
      assert(
        result.effectiveAllowed === true,
        "ENFORCE: baseline + ALLOW grant = allowed",
      );
  ...
  await run(
    "ENFORCE mode: denied by scope even with global permission",
    async () => {
      const originalMode = process.env.AUTHORIZATION_MODE;
      process.env.AUTHORIZATION_MODE = "ENFORCE";
      ...
      assert(
        result.effectiveAllowed === false,
        "ENFORCE: DENY grant should block",
      );
  ...
  await run("ADMIN always allowed", async () => {
    ...
    assert(
      result.reasonCode === "ADMIN_BYPASS",
      "Reason should be ADMIN_BYPASS",
    );
  ...
  await run("ENFORCE: unclassified resource is denied", async () => {
    ...
    assert(
      result.effectiveAllowed === false,
      "ENFORCE: unclassified resource should be denied",
    );
    assert(
      result.reasonCode === "RESOURCE_UNCLASSIFIED" ||
        result.reasonCode === "NO_SCOPE_GRANT",
  ...
  await run("ENFORCE: ambiguous AE title is denied", async () => {
    ...
    assert(
      result.effectiveAllowed === false,
      "ENFORCE: ambiguous AE should be denied",
    );
    assert(
      result.reasonCode === "AMBIGUOUS_MACHINE",
      `Reason should be AMBIGUOUS_MACHINE, got ${result.reasonCode}`,
    );
  ...
  await run(
    "Missing global permission → denied regardless of mode",
    async () => {
      const originalMode = process.env.AUTHORIZATION_MODE;
      process.env.AUTHORIZATION_MODE = "ENFORCE";
      ...
      assert(
        result.effectiveAllowed === false,
        "No global permission → denied",
      );
      assert(
        result.reasonCode === "GLOBAL_PERMISSION_MISSING",
        `Reason should be GLOBAL_PERMISSION_MISSING, got ${result.reasonCode}`,
      );
  ...
  await run(
    "DicomNode grant does not match facility-only resource",
    async () => {
      const originalMode = process.env.AUTHORIZATION_MODE;
      process.env.AUTHORIZATION_MODE = "ENFORCE";
      ...
      assert(
        result.effectiveAllowed === false,
        "Should not be allowed based on dicomNode grant only",
      );
      assert(
        result.reasonCode === "NO_SCOPE_GRANT",
        "Should result in NO_SCOPE_GRANT because no grants matched the resource",
      );
  ...
  await run(
    "ENFORCE mode: denies if machineId is inactive even with performingUnitId and facility ALLOW",
    async () => {
      const originalMode = process.env.AUTHORIZATION_MODE;
      process.env.AUTHORIZATION_MODE = "ENFORCE";
      ...
      assert(
        result.effectiveAllowed === false,
        "Should be denied due to inactive machine",
      );
      assert(
        result.reasonCode === "RESOURCE_INACTIVE",
        `Reason should be RESOURCE_INACTIVE, got ${result.reasonCode}`,
      );
  ...
  await run("Separate contexts do not share cached data", async () => {
    ...
    assert(
      ctx1.getPrincipal("testuser") !== undefined,
      "ctx1 should have cached principal",
    );
    assert(
      ctx2.getPrincipal("testuser") === undefined,
      "ctx2 should NOT have ctx1's principal",
    );
  });
